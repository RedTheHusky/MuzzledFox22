    ...
    protected class runLocal implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if(gEvent.getArgs().isEmpty()){
                    ...
                    else if(items[0].equalsIgnoreCase("play")&&items.length>=2){
                        loadAndPlay(items[1]); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("resume")){
                        resume(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("pause")){
                        pause(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("stop")){
                        stop(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("clear")){
                        clear(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("queue")){
                        showQueue(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("shuffle")){
                        shuffle(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("leave")){
                        leave(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("join")){
                        join(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("volume")&&items.length>=2){
                        volume(items[1]); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("skip")){
                        skip(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("status")){
                        status(); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("register")){
                        registerPlayerManager(true); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("registerforce")){
                        registerForcePlayerManager(true); isInvalidCommand=false;
                    }
                    else if(items[0].equalsIgnoreCase("record")){
                       if(items.length>=2){
                           if(items[1].equalsIgnoreCase("start")){
                               record_start();isInvalidCommand=false;
                           }
                           else if(items[1].equalsIgnoreCase("stop")){
                               record_stop();isInvalidCommand=false;
                           }
                       }else{
                           record_toggle();isInvalidCommand=false;
                       }
                    }
                    else if(items[0].equalsIgnoreCase("add")&&items.length>=2){
                        add2PlayList(items[1]); isInvalidCommand=false;
                    }
                    else if((items[0].equalsIgnoreCase("sub")||items[0].equalsIgnoreCase("rem"))&&items.length>=2){
            ...
            try {
                ...
                gGlobal.playerManager.loadItemOrdered(guildMusicManager, trackUrl, new AudioLoadResultHandler() {
                    ...
                    public void trackLoaded(AudioTrack track) {
                        ...
                        eb=iFlexiPlayer.playTrack(track, eb);
        ...
        private void showQueuePagination(List<AudioTrack>audioTracks,int previous,int next) {
            ...
            try {
                ...
                eb.setColor(new Color(0x7289da));
                eb.setTitle("Player");
                String desc="",tmp="";boolean requirePagination=false;int i=0;
                ...
                if(!requirePagination){
                    ...
                    Message message=lsMessageHelper.lsSendMessageResponse(gTextChannel,eb);
                    //previous
                    //next showQueuePagination(audioTracks,next,i+1)
                }
            }catch (Exception e){
                logger.error(fName + ".exception=" + e);
                logger.error(fName + ".exception:" + Arrays.toString(e.getStackTrace()));
        ...
        public boolean record_start() {
            ...
            try {
                ...
                VoiceChannel channel = gMember.getVoiceState().getChannel();
        ...
        public boolean registerPlayerManager(boolean isCommand) {
            ...
            try {
                ...
                logger.error(fName + ".exception=" + e);
                logger.error(fName + ".exception:" + Arrays.toString(e.getStackTrace()));
                if(isCommand)lsMessageHelper.lsSendQuickErrorEmbedMessageResponse(gTextChannel,gUser,gTitle,e.toString());
                return false;
            }
        }
