...
public class SantaLetters extends Command implements llMessageHelper, llGlobalHelper, llMemberHelper, llNetworkHelper {
    Logger logger = Logger.getLogger(getClass());Logger logger2 = Logger.getLogger("SentSantaLetters");
    lcGlobalHelper gGlobal;
    String gTitle="Dear Santa",gCommand="dearsanta";
    public SantaLetters(lcGlobalHelper global){
        String fName="[constructor]";
        logger.info(fName);
        gGlobal=global;
        this.name = gTitle;
        this.help = "Confessing to Santa what an angelic or bad person you have been this year.";
        this.aliases = new String[]{gCommand};
        this.botPermissions = new Permission[]{Permission.MESSAGE_EMBED_LINKS};
        this.guildOnly = true;this.category= llCommandCategory_SFW;this.hidden=true;
    ...
    protected class runLocal implements Runnable {
                        ...
                        if(items[0].equalsIgnoreCase("help")){
                           ...
                           display();isInvalidCommand=false;
                            ...
                            if(gTextChannel.isNSFW()){
                                logger.warn(fName+"can't be nsfw channel");
                                lsMessageHelper.lsSendQuickEmbedMessage(gUser,gTitle,"This is the SFW version. Can't use it in NSFW channel!", llColors_Orange.llColorOrange_InternationalEngineering);
                            }else{
                                requestLetter();
                            }
                           isInvalidCommand=false;
        ...
        private void help( String command) {
            ...
            if(command.equalsIgnoreCase("setup")){
                ...
                desc="Confess what an angelic or bad person you have been this year." +
                        "\n\u0095 All confessions are anonymous." +
                        "\n\u0095 Only use the bot for it's intended purpose." +
                        "\n\u0095 This is the SFW version. For the NSFW check it under the NSFW commands.";
        ...
        private void requestLetter() {
            ...
            try {
                ...
                if(postChannel.isNSFW()){
                    logger.warn(fName+"can't be nsfw channel");
                    lsMessageHelper.lsSendQuickEmbedMessage(gUser,gTitle,"This is the SFW version. Can't post it to NSFW channel!", llColors_Orange.llColorOrange_InternationalEngineering);
                    return;
                }
                try {
                    if(!whiteRoles.isEmpty()){
        ...
        private void postLetter(long postId,String content) {
            ...
            try {
               ...
               if(message!=null){
                   ...
                   logger2.log(CrunchifyLog4jLevel.SENTSANTALETTER,"Author="+gUser.getName()+"#"+gUser.getDiscriminator()+"("+gUser.getId()+"), guild="+gGuild.getName()+"("+gGuild.getId()+"), message="+content);
               }
            }catch (Exception e){
                logger.error(fName + ".exception=" + e);
                logger.error(fName + ".exception:" + Arrays.toString(e.getStackTrace()));
        ...
        String gProfileName ="SantaLetters";
