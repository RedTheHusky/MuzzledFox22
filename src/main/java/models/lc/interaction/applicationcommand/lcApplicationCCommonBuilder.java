...
public class lcApplicationCCommonBuilder {
    Logger logger = Logger.getLogger(getClass());
    public lcApplicationCCommonBuilder(){

    }
    public lcApplicationCCommonBuilder(lcGlobalHelper global){
        this.global=global;
    }
    ...
    public lcApplicationCCommonBuilder(JSONObject jsonObject) {
        String fName = "[build]";
        try {
    ...
    public lcApplicationCCommonBuilder(JSONObject jsonObject, Guild guild) {
        String fName = "[build]";
        try {
            logger.info(fName + ".creating");
            set(jsonObject);
    ...
    public lcApplicationCCommonBuilder set(JSONObject jsonObject) {
        String fName = "[set]";
        try {
    ...
    protected  class _GENERAL{
        public _GENERAL(){}
        public _GENERAL set(JSONObject jsonObject) {
            String fName = "[set]";
            try {
                if(jsonObject==null)throw  new Exception("Json is null!");
                if(jsonObject.isEmpty())throw  new Exception("Json is empty!");
                String key="";
                key= llCommonKeys.ApplicationCommandStructure.Type;if(jsonObject.has(key)&&!jsonObject.isNull(key))OTHER.setType(jsonObject.optInt(key,-1));
                key= llCommonKeys.ApplicationCommandStructure.Name;if(jsonObject.has(key)&&!jsonObject.isNull(key))setName(jsonObject.optString(key,""));
                key= llCommonKeys.ApplicationCommandStructure.DefaultPermission;if(jsonObject.has(key)&&!jsonObject.isNull(key))setDefaultPermission(jsonObject.optBoolean(key));
                logger.info(fName+"none failed");
                return  this;
            } catch (Exception e) {
    ...
    public lcApplicationCCommonBuilder setName(String input) {
        String fName = "[setName]";
        try {
            logger.info(fName + ".input=" + input);
            if(input==null)throw  new Exception("Cant be null!");
    ...
    public lcApplicationCCommonBuilder setDefaultPermission(boolean input) {
    ...
    public lcApplicationCCommonBuilder setGuild(Guild guild) {
    ...
    public lcApplicationCCommonBuilder clearGuild() {
    ...
    public JSONObject getJson() {
        ...
        try {
            if(getType()== liApplicationCommand.lcApplicationTypes.CHAT_INPUT){
                return SLASH.getJson();
            }else{
                return GENERAL.getJson();
            }
        } catch (Exception e) {
            logger.error(fName + ".exception=" + e);
            logger.error(fName + ".exception:" + Arrays.toString(e.getStackTrace()));
    ...
    public JSONObject getJson4Build() {
        ...
        try {
            if(getType()== liApplicationCommand.lcApplicationTypes.CHAT_INPUT){
                return SLASH.getJson4Build();
            }else{
                return GENERAL.getJson4Build();
            }
        } catch (Exception e) {
            logger.error(fName + ".exception=" + e);
            logger.error(fName + ".exception:" + Arrays.toString(e.getStackTrace()));
    ...
    private boolean postGuild(JSONObject jsonObject){
    ...
    private boolean postGlobal(JSONObject jsonObject){
    ...
    private boolean post(JSONObject jsonObject,String url){
        String fName="[post:Url]";
        try {
    ...
    public lcApplicationCCommonEditor post(){
        String fName="[post]";
        try {
    ...
    public lcApplicationCCommonEditor post(JSONObject jsonObject){
        String fName="[post]";
        try {
            boolean response=false;
            logger.info(fName+".selection");
            if(getGuild()!=null){
                response=postGuild(jsonObject);
            }else{
                response=postGlobal(jsonObject);
            }
            logger.info(fName+".response="+response);
            if(response){
                return  new lcApplicationCCommonEditor(postResponse.getBody().getObject(),getGuild());
            }
            return null;
        }catch (Exception e){
            logger.error(fName + ".exception=" + e);
