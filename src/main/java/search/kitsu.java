...
public class kitsu extends Command implements llMessageHelper, llGlobalHelper, llMemberHelper {
    String gUrlSearch ="https://kitsu.io/api/edge/!main?page[limit]=20";
    String filterAnime="anime", filterManga ="manga",filterCategory="filter[categories]=",filterText="filter[text]=";
    Logger logger = Logger.getLogger(getClass());
    lcGlobalHelper gGlobal;CommandEvent gEvent;
    String gTitle="kitsu-Searcher", gCommand ="kitsu";
    User gUser;
    Member gMember;
    Guild gGuild;
    TextChannel gTextChannel;
    EventWaiter gWaiter;
    public kitsu(lcGlobalHelper global){
        String fName="[constructor]";
        logger.info(fName);
        gGlobal=global;gWaiter=global.waiter;
        this.name = gTitle;
        this.help = "Search anime/manga from Kitsu";
        this.aliases = new String[]{"kitsu"};
    ...
    protected class runLocal implements Runnable {
        ...
        public void run() {
            ...
            try {
                gBasicFeatureControl=new lcBasicFeatureControl(gGuild,"kitsu",gGlobal);
            ...
            /*if(!isNSFW()){
                ...
                if(gEvent.getArgs().isEmpty()){
                    ...
                    else if(items.length>1&&items[0].equalsIgnoreCase("manga")){
                        String tmp="";
                        for(int i=1;i<items.length;i++){
                            tmp+=" "+items[i];
                        }
                        tmp=tmp.trim();
                        doSearch(tmp,filterManga);
                    }
                    else if(items.length>1&&items[0].equalsIgnoreCase("anime")){
                        String tmp="";
                        for(int i=1;i<items.length;i++){
                            tmp+=" "+items[i];
                        }
                        tmp=tmp.trim();
                        doSearch(tmp,filterAnime);
                    }
                    else{
                        options.put(keyFamily,filterAnime);
                        doSearch(gEvent.getArgs(),filterAnime);
                    }
        ...
        private void setOptions(String message) {
            ...
            try{
                ...
                if(convertedMessage.contains(begin)){
                    ...
                    l=begin.length();
                    j=convertedMessage.indexOf("]");
                    options.put(keyText,convertedMessage.substring(i+l,j));
                    StringBuffer stringBuffer = new StringBuffer(convertedMessage);
                    stringBuffer.replace( i ,j+1,"");
                    logger.info(fName+"keyText="+options.getString(keyText));
                    convertedMessage=stringBuffer.toString();
                    logger.info(fName+"convertedMessage="+convertedMessage);
                }
                //convertedMessage=convertedMessage.replaceFirst(filterAnime,"").replaceFirst(filterManga,"");
                if(convertedMessage.contains(keyCategory)){
                    options.put(keyCategory,convertedMessage);convertedMessage=convertedMessage.replaceAll(keyCategory,"");
                }else{
        ...
        private void getHtmlGallerySearch() {
            ...
            try {
                ...
                url=url.replaceAll("!main",options.getString(keyFamily));
                if(options.has(keyText)&&!options.isNull(keyText)){
                    url=url+"&"+filterText+options.getString(keyText);
                }
                if(options.has(keyCategory)&&!options.isNull(keyCategory)){
                    url=url+"&"+filterCategory+options.getString(keyCategory);
                }
        ...
        private void doSearch(String message,String family) {
            String fName = "[doSearch]"; logger.info(fName);options=new JSONObject();
            try{
                logger.info(fName+"message="+message);
                logger.info(fName+"family="+family);
                options.put(keyFamily,family);
                setOptions(message);
        ...
        String keyId="id";
        String keyattributes="attributes",keynsfw="nsfw",keycanonicalTitle="canonicalTitle",keyaverageRating="averageRating",keyratingRank="ratingRank",keystatus="status",keyageRatingGuide="ageRatingGuide",keysynopsis="synopsis";
        String keyPosterImage="posterImage",keymedium="medium",keylarge="large",keyoriginal="original",keysmall="small";
        String keyLinks="links",keySelf="self";
        private EmbedBuilder createEmbedDisplay(){
            ...
            try {
                ...
                String title="#",self="";
                JSONObject attributes=new JSONObject();boolean isNsfw=false;
                if(selectedItem.has(keyattributes)&&!selectedItem.isNull(keyattributes)) {
                    attributes = selectedItem.getJSONObject(keyattributes);
                    if (attributes.has(keynsfw)) {
                        isNsfw = attributes.getBoolean(keynsfw);
                    }
                    if (!gTextChannel.isNSFW() && isNsfw) {
                        embedBuilder.setDescription("Whops!!! This anime/manga is labeled NSFW bit the channel is not labeled NSFW!");
                        embedBuilder.setColor(llColorRed_Carmine);
                    } else {
                        if (attributes.has(keycanonicalTitle)&&!attributes.isNull(keycanonicalTitle)) {
                            title = attributes.getString(keycanonicalTitle);
                        }
                        if (attributes.has(keysynopsis)&&!attributes.isNull(keysynopsis)) {
                            embedBuilder.setDescription(attributes.getString(keysynopsis));
                        }
                        embedBuilder.setTitle(title);
                        if (attributes.has(keyageRatingGuide)&&!attributes.isNull(keyageRatingGuide)) {
                            embedBuilder.addField("Age Rating", attributes.getString(keyageRatingGuide), true);
                        }
                        if (attributes.has(keystatus)&&!attributes.isNull(keystatus)) {
                            embedBuilder.addField("Status", attributes.getString(keystatus), true);
                        }
                        if (attributes.has(keyaverageRating)&&!attributes.isNull(keyaverageRating)) {
                            embedBuilder.addField("Average Rating", attributes.getString(keyaverageRating), true);
                        }
                        if (attributes.has(keyratingRank)&&!attributes.isNull(keyratingRank)) {
                            embedBuilder.addField("Rating", attributes.getString(keyratingRank), true);
                        }
                        if (attributes.has(keyPosterImage) && !attributes.isNull(keyPosterImage)) {
                            JSONObject object = attributes.getJSONObject(keyPosterImage);
                            if (object.has(keymedium) && !object.isNull(keymedium)) {
                                embedBuilder.setThumbnail(object.getString(keymedium));
                            } else if (object.has(keylarge) && !object.isNull(keylarge)) {
                                embedBuilder.setThumbnail(keylarge);
                            } else if (object.has(keyoriginal) && !object.isNull(keyoriginal)) {
                                embedBuilder.setThumbnail(keylarge);
                            } else if (object.has(keyoriginal) && !object.isNull(keyoriginal)) {
                                embedBuilder.setThumbnail(keyoriginal);
                            }
                        }
                    }
                }
                if(selectedItem.has(keyLinks)&&!selectedItem.isNull(keyLinks)){
                    if (selectedItem.getJSONObject(keyLinks).has(keySelf)&&!selectedItem.getJSONObject(keyLinks).isNull(keySelf)) {
                        self =selectedItem.getJSONObject(keyLinks).getString(keySelf);
                        embedBuilder.setTitle(title,self);
                    }
                }
                return embedBuilder;
            } catch (Exception e) {
                logger.error(fName+"exception:"+e);
                logger.error(fName+ ".exception:" + Arrays.toString(e.getStackTrace()));
                llSendQuickEmbedMessage(gTextChannel,gTitle,"Error!",llColorRed);return  new EmbedBuilder();
            }
        }
        private void reactionMenu(){
            ...
            logger.info(fName);
            try{
                EmbedBuilder embedBuilder=createEmbedDisplay();
                Message message=lsMessageHelper.lsSendMessageResponse(gTextChannel,embedBuilder.build());
                            ...
                            e -> {
                                try {
                                    ...
                                    if(nameCode.equalsIgnoreCase(gGlobal.emojis.getEmoji(lsUnicodeEmotes.aliasReverseButton))){
                                        ...
                                        logger.info(fName+"finalIndex="+finalIndex);
                                        if((finalIndex-1)>=0){
