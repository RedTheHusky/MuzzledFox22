...
import userprofiles.entities.rCharacter;
import userprofiles.entities.upExtensionCharacter;

import java.util.Arrays;
import java.util.List;

public class character extends Command implements llMessageHelper, llGlobalHelper, llMemberHelper, rCharacter {
    Logger logger = Logger.getLogger(getClass());
    lcGlobalHelper gGlobal;
    String KeyTag ="character", gTitle="Character",profileName="characters";
    public character(lcGlobalHelper global){
        String fName="[constructor]";
        logger.info(fName);
        gGlobal=global;
        this.name = "Character";
        this.help = "Creating/Displaying your character(s). SFW edition";
        this.aliases = new String[]{KeyTag,"characters"};
        this.guildOnly = true;this.category= llCommandCategory_SFW;
        this.hidden=false;
    }
    public character(lcGlobalHelper global, SlashCommandEvent event){
        String fName="[constructor]";
        logger.info(fName);
        gGlobal=global;
        if(gGlobal==null)logger.error("global is null at 0");
    ...
    protected class runLocal extends upExtensionCharacter implements Runnable {

        String cName = "[runLocal]";
        public runLocal(CommandEvent ev) {
            logger.info(".run build");
            KeyTag ="character";gTitle="Character";profileName="characters";
            launch(gGlobal,ev);
        }
        ...
        public void run() {
            ...
            try {
                gBasicFeatureControl=new lcBasicFeatureControl(gGuild,"character",gGlobal);
                ...
                if(gSlashCommandEvent!=null){
                    ...
                    if(gCommandEvent.getArgs().isEmpty()){
                        ...
                        if(isInvalidCommand){
                            ...
                            if(gItems[0].equalsIgnoreCase("stop")){
                            ...
                            if(gItems[0].equalsIgnoreCase("use")){
                                if(gItems.length<2){
                                    noSlot();isInvalidCommand=false;
                                }else{
                                    use(gItems[1]);isInvalidCommand=false;
                                }
                            }else
    ...
    public character(lcGlobalHelper global, Guild guild, User user,String command){
        logger.info(".run build");
        Runnable r = new runUtility(global,guild,user,command);
        new Thread(r).start();
    }
    protected class runUtility extends upExtensionCharacter implements Runnable {
