...
public class confessionalBooth extends Command implements llMessageHelper, llGlobalHelper, llMemberHelper, llNetworkHelper {
    Logger logger = Logger.getLogger(getClass());Logger logger2 = Logger.getLogger("ConfessionalBooth");
    lcGlobalHelper gGlobal;
    String gTitle="Confessional Booth",gCommand="confessional";
    public confessionalBooth(lcGlobalHelper global){
        String fName="[constructor]";
        logger.info(fName);
        gGlobal=global;
        this.name = gTitle;
        this.help = "Confessing your naughtiness.";
        this.aliases = new String[]{gCommand,"darkbox","confessionalbooth"};
    ...
    protected class runLocal implements Runnable {
        ...
        private void help( String command) {
            ...
            if(command.equalsIgnoreCase("setup")){
                ...
                desc+="\n`"+quickSummonWithSpace+"channel set <textchannel mention>` to set the channel where all confessions will be sent.";
                desc+="\n`"+quickSummonWithSpace+"channel clear` to remove so said channel.";
                desc+="\n`"+quickSummonWithSpace+"roles set [list of mentioned roles]` to set only members with this roles can submit a confession.";
                ...
                desc+="\n`"+quickSummonWithSpace+"roles clear` to clear the roles list, allowing anyone to submit a confession.";
            }
                ...
                desc="Confess what an naughty person you have been." +
                        "\n\u0095 All confessions are anonymous." +
                        "\n\u0095 Only use the bot for it's intended purpose." +
                        "\n\u0095 This is intended for NSFW confessions, so will only work in nsfw channels.";
        ...
        private void setEnable(String command) {
            ...
            try {
                logger.info(fName+"command="+command);
                if(!lsMemberHelper.lsMemberHasPermission_MANAGESERVER(gMember)&&!lsMemberHelper.lsMemberIsAdministrator(gMember)){
                    logger.warn(fName+"denied");
                    lsMessageHelper.lsSendQuickEmbedMessage(gUser,gTitle,"You don't have the permission manage server!", llColors_Orange.llColorOrange_InternationalEngineering);
                    return;
                }
                getValues();
            ...
            try {
                ...
                if(command.equalsIgnoreCase("set")){
                    ...
                    if(!textChannelList.get(0).isNSFW()){
                        ...
                        lsMessageHelper.lsSendQuickEmbedMessage(gUser,gTitle,"Can't use it in SFW channel!", llColors_Orange.llColorOrange_InternationalEngineering);
                        return;
                    }
        ...
        private void requestLetter() {
            ...
            try {
                ...
                if(!postChannel.isNSFW()){
                    logger.warn(fName+"can't be sfw channel");
                    lsMessageHelper.lsSendQuickEmbedMessage(gUser,gTitle,"Can't post it to SFW channel!", llColors_Orange.llColorOrange_InternationalEngineering);
                    return;
                }
                try {
                    if(!whiteRoles.isEmpty()){
